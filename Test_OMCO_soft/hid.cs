...
namespace HID
{
    ...
    public class Hid : Object
    {
        ...
        public FileStream  hidDevice = null; //异步IO流
        ...
        static public HID_RETURN GetDeviceSerialList(UInt16 vID,UInt16 pID,ref List<string> serialList)
        {
            ...
            for (int i = 0; i < deviceList.Count; i++)
            {
                ...
                if (device != INVALID_HANDLE_VALUE)
                {
                    ...
                    IntPtr ProductBuff = Marshal.AllocHGlobal(512);
                    HidD_GetProductString(device, ProductBuff, 512);
                    string ProductStr = Marshal.PtrToStringAuto(ProductBuff);
        ...
        /// <param name="product">设备的产品名字</param>
        /// <returns></returns>
        public IntPtr OpenDevice(UInt16 vID, UInt16 pID, string product)
        {
            ...
            if (deviceOpened == false)
            {
                //获取连接的HID列表
                List<string> deviceList = new List<string>();
                GetDeviceSerialList(vID, pID, ref deviceList);
                GetHidDeviceList(ref deviceList);
                if (deviceList.Count == 0)
                ...
                for (int i = 0; i < deviceList.Count; i++)
                {
                    ...
                    if (device != INVALID_HANDLE_VALUE)
                    {
                        ...
                        IntPtr SerialBuff = Marshal.AllocHGlobal(512);
                        HidD_GetSerialNumberString(device, SerialBuff, 512);
                        string SerialStr = Marshal.PtrToStringAnsi(SerialBuff);
                        Marshal.FreeHGlobal(SerialBuff);

                        Debug.WriteLine(attributes.VendorID);
                        Debug.WriteLine(attributes.ProductID);
                        if (attributes.VendorID == vID && attributes.ProductID == pID && ProductStr == product)
                        {
                            IntPtr preparseData;
                            HIDP_CAPS caps;
        ...
        /// <returns></returns>
        [DllImport("hid.dll")]
        private static extern Boolean HidD_GetProductString(IntPtr hidDeviceObject, IntPtr buffer, int bufferLength);
